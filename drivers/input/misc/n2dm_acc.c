...
#define SENSITIVITY_2G		3096	/**	ug/LSB	*/
#define SENSITIVITY_4G		7813	/**	ug/LSB	*/
#define SENSITIVITY_8G		15625	/**	ug/LSB	*/
#define SENSITIVITY_16G		46875	/**	ug/LSB	*/
...
#define N2DM_ACC_ODR1620		(0x80)  /* 1620Hz output data rate *///xym add 
...
/*#if defined(CONFIG_BOARD_ABBY)//P890A57 unused
	.axis_map_x = 0,
	.axis_map_y = 1,
	.axis_map_z = 2,
	.negate_x = 1,
							...
							int poll_interval_ms)
{
	...
	int i;
	u8 config[2];
	//dev_info(&stat->client->dev, "%s: %d ms\n", __func__, poll_interval_ms);
...
#if 1//xym add for n2dm'odr, begin
	{
		u8 data = CTRL_REG1;
		...
		switch (data) {
		case N2DM_ACC_ODR1:
			dev_info(&stat->client->dev, "ODR = 1 Hz\n");
			break;
		case N2DM_ACC_ODR10:
			dev_info(&stat->client->dev, "ODR = 10 Hz\n");
			break;
		case N2DM_ACC_ODR25:
			dev_info(&stat->client->dev, "ODR = 25 Hz\n");
			break;
		case N2DM_ACC_ODR50:
			dev_info(&stat->client->dev, "ODR = 50 Hz\n");
			break;
		case N2DM_ACC_ODR100:
			dev_info(&stat->client->dev, "ODR = 100 Hz\n");
			break;
		case N2DM_ACC_ODR200:
			dev_info(&stat->client->dev, "ODR = 200 Hz\n");
			break;
		case N2DM_ACC_ODR400:
			dev_info(&stat->client->dev, "ODR = 400 Hz\n");
			break;
		case N2DM_ACC_ODR1620:
			dev_info(&stat->client->dev, "ODR = 1620 Hz\n");
			break;
		case N2DM_ACC_ODR1344:
			dev_info(&stat->client->dev, "ODR = 1340 Hz\n");
			break;
		default:
			dev_info(&stat->client->dev, "Unknown ODR. data=0x%X\n", data);
			break;
		}
	}
#endif//xym add for n2dm'odr, end
					...
					int *xyz)
{
    ...
    xyz[0]=(xyz[0]*16)/SENSITIVITY_2G;
	xyz[1]=(xyz[1]*16)/SENSITIVITY_2G;
	xyz[2]=(xyz[2]*16)/SENSITIVITY_2G;
    //---
				     ...
				     const char *buf, size_t size)
{
	...
	if (!interval_ms)
		return -EINVAL;
	dev_info(&stat->client->dev, "%s: %ld ms\n", __func__, interval_ms);
				...
				unsigned int delay_ms)
{
	...
	pr_info("%s: %d ms\n", __func__, delay_ms);
	n2dm_acc_update_odr(data, delay_ms);//xym add for n2dm's odr
 ...
 static int n2dm_acc_parse_dt(struct device *dev, struct n2dm_acc_platform_data *pdata)
{
	...
	u32 temp_val;
	int rc;
	rc = of_property_read_u32(np, "n2dm,axis-map-x", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "n2dm,axis-map-y", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	rc = of_property_read_u32(np, "n2dm,axis-map-z", &temp_val);
	if (rc && (rc != -EINVAL)) {
	...
	pdata->negate_x = of_property_read_bool(np, "n2dm,negate-x");
	pdata->negate_y = of_property_read_bool(np, "n2dm,negate-y");
	pdata->negate_z = of_property_read_bool(np, "n2dm,negate-z");
	...
	//pdata->gpio_int1 = of_get_named_gpio(dev->of_node, "n2dm,gpio-int1", 0);
	//pr_info("%s: gpio_int1=%d\n",__func__, pdata->gpio_int1);
	//pdata->gpio_int2 = of_get_named_gpio(dev->of_node, "n2dm,gpio-int2", 0);
	//pr_info("%s: gpio_int2=%d\n",__func__, pdata->gpio_int2);
	...
	if (rc && (rc != -EINVAL)) {
		...
		case 2:
			...
			break;
		case 4:
			pdata->fs_range = N2DM_ACC_G_4G;
			break;
		case 8:
			pdata->fs_range = N2DM_ACC_G_8G;
			break;
		case 16:
			pdata->fs_range = N2DM_ACC_G_16G;
			break;
		default:
			pdata->fs_range = N2DM_ACC_G_2G;
			break;
	...
	pr_info("%s: min_interval=%d,\n\t  poll_interval=%d\n",__func__, pdata->min_interval, pdata->poll_interval);
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		//memset(stat->pdata, 0 , sizeof(stat->pdata));
		memset(stat->pdata, 0 , sizeof(*stat->pdata));		
	...
	/*dev_info(&client->dev, "xym temp stat->pdata->init=0x%X\n", (int)stat->pdata->init);
	dev_info(&client->dev, "xym temp stat->pdata->exit=0x%X\n", (int)stat->pdata->exit);
	dev_info(&client->dev, "xym temp stat->pdata->power_on=0x%X\n", (int)stat->pdata->power_on);
	dev_info(&client->dev, "xym temp stat->pdata->power_off=0x%X\n", (int)stat->pdata->power_off);*/
	//xym add end
